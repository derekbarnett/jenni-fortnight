...
def weekcheck1(jenni, t, generic):
...
def weekcheck2(jenni, t, generic):
...
def nextshow(jenni, input):
    ...
    if not schedule == 1 and not schedule == 2:
        return jenni.reply("The type of schedule to track is not properly set.")
    ...
    #are looking for
    if schedule == 1:
        showmessage = weekcheck1(jenni, t, generic)
    elif schedule == 2:
        showmessage = weekcheck2(jenni, t, generic)
